...
from transformers import DistilBertTokenizerFast, DistilBertForQuestionAnswering, AutoTokenizer, AutoModelForQuestionAnswering, AdamW
...
def read_and_process(args, tokenizer, dataset_dict, split):
...
class Trainer:
    ...
    def test_moe(self, model, eval_loader, eval_dict, save_dir, dev_loader, dev_dict):
        eval_preds, eval_scores = self.evaluate_moe(
            model, eval_loader, eval_dict, return_preds=True, split="test"
        )
        ...
        eval_preds, eval_scores = self.evaluate_moe(
            model, dev_loader, dev_dict, return_preds=True, split="test"
        )
        results_str = ", ".join(f"{k}: {v:05.2f}" for k, v in eval_scores.items())
        self.log.info(f"Eval {results_str}")
        # Write submission file
    ...
    def train(
        ...
        for epoch_num in range(self.num_epochs):
            ...
            with torch.enable_grad(), tqdm(
                total=int(len(train_dataloader.dataset))
            ) as progress_bar:
    ...
    def train_moe(
        ...
        if pretrain_dataloader is not None:
            ...
            for epoch_num in range(self.num_epochs_pretrain):
                ...
                with torch.enable_grad(), tqdm(
                    total=len(pretrain_dataloader.dataset)
                ) as progress_bar:
                    for batch in pretrain_dataloader:
                        ...
                        if rank == 0:
                            ...
                            progress_bar.set_postfix(epoch=epoch_num, NLL=loss.item())
                            tbx.add_scalar(
                                "pretrain/NLL", loss.item(), pretrain_step_idx
                            )
                            wandb.log(
                                {
                        ...
                        if (
                            pretrain_step_idx >= pretrain_eval_count * 10000
                        ) and rank == 0:
                            ...
                            for k, v in curr_score.items():
                                tbx.add_scalar(
                                    f"pretrain_oodomain_val/{k}", v, pretrain_step_idx
                                )
...
def get_dataset(args, tokenizer, split_name, num_aug=0):
    ...
    data_encodings = read_and_process(
        args, tokenizer, dataset_dict, split_name
    )

    return (
        util.QADataset(data_encodings, train=(split_name in ["train", "finetune"])),
        dataset_dict,
    )
...
def main(rank, world_size, args):
    assert args.model_type in [
        ...
        "biobert",
        "moe",
    ], "model must be either distilbert, biobert, moe"
    ...
    if rank == 0:
        run = wandb.init(project="biomedicalqa", entity="cs5980-umn")
    else:
        run = None
    ...
    if args.model_type == "distilbert":
        ...
        raise ValueError(
            "model_type must be either distilbert, biobert, moe"
        )
    if world_size > 1:
    ...
    if args.do_train:
        ...
        if args.pretrain:
            ...
            pretrain_dataset, _ = get_dataset(
                args, tokenizer, "train", args.num_aug_pretrain
            )
        ...
        if args.pretrain:
            pretrain_sampler = (
                RandomSampler(pretrain_dataset)
                if world_size == 1
                else torch.utils.data.distributed.DistributedSampler(pretrain_dataset)
            )
            pretrain_loader = DataLoader(
                pretrain_dataset, batch_size=args.batch_size, sampler=pretrain_sampler
            )
        else:
        ...
        test_dict = None
        if rank == 0:
            ood_val_dataset, ood_val_dict = get_dataset(args, tokenizer, "ood_val")
        ...
        if args.model_type in ["distilbert", "biobert"] :
